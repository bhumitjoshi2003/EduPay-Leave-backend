...
import com.indraacademy.ias_management.config.PaymentPricingProperties;
import org.springframework.stereotype.Component;

/** Deterministic, paise-native calculator for ONLINE_CONVENIENCE_FEE_V1. */
@Component
public class OnlinePaymentPricingCalculator {
    ...
    private final PaymentPricingProperties properties;

    public OnlinePaymentPricingCalculator(PaymentPricingProperties properties) {
        this.properties = properties;
    }

    public Pricing calculate(long schoolLiabilityPrincipalPaise, long otherCapturedNonConveniencePaise) {
        return calculate(schoolLiabilityPrincipalPaise, otherCapturedNonConveniencePaise,
                properties.getGateway().getRateBps(), properties.getGateway().getTaxRateBps(),
                properties.getEdunexifyTransactionFeePaise());
    }

    static Pricing calculate(long principal, long other, int rateBps, int taxRateBps, long transactionFee) {
